...
Sprite = function () {
  ...
  this.move = function (delta) {
    ...
    var delta_curved = delta + this.Curvature();
    if ($.isFunction(this.preMove)) {
      this.preMove(delta);
    }

    this.vel.x += this.acc.x * delta_curved;
    this.vel.y += this.acc.y * delta_curved;
    this.x += this.vel.x * delta_curved;
    this.y += this.vel.y * delta_curved;
    this.rot += this.vel.rot * delta_curved;
    if (this.rot > 360) {
      this.rot -= 360;
    } else if (this.rot < 0) {
      this.rot += 360;
  ...
  this.wrapPostMove = function () {
    ...
    if (this.y > Game.canvasHeight) {
      this.y = 0;
      this.x = Game.canvasWidth - this.x;
      this.FlipY();
    } else if (this.y < 0) {
  ...
  this.FlipRotX = function()
  {
      var xComp = -Math.cos(this.rot);
      var yComp = Math.sin(this.rot);
      var newRot = this.calcAngleDegrees(xComp, yComp);
      this.rot = newRot;
  };

  this.FlipRotY = function()
  {
      var xComp = Math.cos(this.rot);
      var yComp = -Math.sin(this.rot);
      var newRot = this.calcAngleDegrees(xComp, yComp);
    ...
    this.calcAngleDegrees = function(x,y)
    {
    ...
    this.Curvature = function()
    {
        var curveMod = 0.003;
        return curveMod * this.Radius();
    };

    this.Radius = function()
    {
        var x = (this.x - Game.canvasWidth/2);
        var y = (this.y - Game.canvasHeight/2);
